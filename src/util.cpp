...
const char * const BITCOIN_CONF_FILENAME = "corebizcoin.conf";
const char * const BITCOIN_PID_FILENAME = "corebizcoin.pid";
...
#else
    const char* pszModule = "corebizcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CoreBizCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CoreBizCoin";
#else
    // Unix
    return pathRet / ".corebizcoin";
#endif
#endif
}
